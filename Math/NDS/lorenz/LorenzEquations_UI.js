...
function OnStartBtnClickCB () {
  if (_IsPlaying == _PlayMode_STOPPED) {
    ...
    document.getElementById ('startBtn').style.backgroundColor = '#992600'; //'#cc3300';
    DisableUI (true);
    animate ();
    ...
    document.getElementById ('startBtn').style.color = '#fff';
    document.getElementById ('startBtn').style.backgroundColor = '#006622'; //'#009933';
    DisableUI (false);
...
function OnResetBtnClickCB () {
  if (_IsPlaying == _PlayMode_STOPPED) {
    ...
    document.getElementById ('initPosOutput').value = '(' + _Default_XInitPos.toFixed(1) + ', '
                                                          + _Default_YInitPos.toFixed(1) + ', '
                                                          + _Default_ZInitPos.toFixed(1) + ')';
...
//
// DisableUI: toggle various UI elements.  Note when playing, the different initial
//            point UI elements are disabled so that the values can not be changed
//            since they can alter the simulation and confuse the user
//
