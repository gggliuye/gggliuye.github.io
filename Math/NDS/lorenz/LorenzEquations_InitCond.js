...
//
// MakeGeomSolPnts: make the three.js geometry for the solution points.  The positions are
...
function ComputeInitialPoints ()
{
    ...
    for (var i = 0; i < _nNumInitPoints; i++)
    {
        ...
        if (_InitPntGeom == _PointGeometry_XAxis)
            _InitPos.push ( [ _InitPosBase[0] + offset, _InitPosBase[1], _InitPosBase[2] ] );
        else if (_InitPntGeom == _PointGeometry_YAxis)
            _InitPos.push ( [ _InitPosBase[0], _InitPosBase[1] + offset, _InitPosBase[2] ] );
        else if (_InitPntGeom == _PointGeometry_ZAxis)
            _InitPos.push ( [ _InitPosBase[0], _InitPosBase[1], _InitPosBase[2] + offset ] );
    }
}


//
// UpdateColours... update the colours of the solution line and point visualization
//
